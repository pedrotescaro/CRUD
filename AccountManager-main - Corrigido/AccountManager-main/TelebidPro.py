...
# Aqui vai o restante do código, incluindo as funções atualizadas para recuperação de senha, envio de e-mail e interface do usuário.
...
# Carregar as imagens e redimensionar para 50x50 pixels
sun_image = Image.open("sun.png").resize((45, 45))
moon_image = Image.open("moon.png").resize((45, 45))

# Remover o fundo branco das imagens da lua
moon_image = moon_image.convert("RGBA")
datas = moon_image.getdata()


# Converter as imagens em objetos PhotoImage
sun_icon = ImageTk.PhotoImage(sun_image)
moon_icon = ImageTk.PhotoImage(moon_image)

# Defina as cores para os temas claro e escuro
light_theme = {"bg": "#ffffff", "fg": "white", "button_bg": "blue"}
dark_theme = {"bg": "#1f1f1f", "fg": "white", "button_bg": "#333333"}

# Inicialize o tema atual como claro
current_theme = light_theme

def apply_theme():
    root.config(bg=current_theme["bg"])
    login_button_menu.config(bg=current_theme["button_bg"], fg=current_theme["fg"])
    register_button_menu.config(bg=current_theme["button_bg"], fg=current_theme["fg"])
    exit_button.config(bg=current_theme["button_bg"], fg=current_theme["fg"])
    toggle_theme_button.config(image=sun_icon if current_theme == light_theme else moon_icon, bd=0, highlightthickness=0, bg=current_theme["bg"])  # Substituir o texto pelo ícone de sol ou lua, remover a borda e ajustar o fundo
...
# Criar o botão de alternar tema com o ícone de sol
toggle_theme_button = tk.Button(root, image=sun_icon, command=toggle_theme, bd=0, highlightthickness=0, bg="#ffffff")  # Inicialmente com fundo branco
toggle_theme_button.place(x=1540, y=10)  # Posicionar no canto superior direito da tela
